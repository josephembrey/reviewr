...
if [ -n "${REVIEWR_BIN:-}" ]; then
  REVIEWR="$REVIEWR_BIN"
elif [ -n "${HERDR_PLUGIN_ROOT:-}" ]; then
  REVIEWR="$HERDR_PLUGIN_ROOT/bin/reviewr"
else
  REVIEWR="reviewr"
fi
...
if [ -n "${HERDR_PLUGIN_ROOT:-}" ] && [ -x "$HERDR_PLUGIN_ROOT/bin/reviewr" ]; then
  for link_dir in "$HOME/.local/state/herdr/plugins/persiyanov.reviewr/bin" "$HOME/.local/bin"; do
    ...
    if [ -L "$link_dir/reviewr" ] || [ ! -e "$link_dir/reviewr" ]; then
      ln -sfn "$HERDR_PLUGIN_ROOT/bin/reviewr" "$link_dir/reviewr" 2>/dev/null || :
    fi
  done
fi
...
# decides, never `name`: that field is a rewritable process title (legacy/docs/herdr-api-notes.md).
...
is_reviewr_pane() {
  ...
  count=$(printf '%s' "$info" | jq -r '
    ...
    [.result.process_info.foreground_processes[]
      | select((((.argv0 // "") | base) == "reviewr")
          or ((((.argv // [])[0] // "") | base) == "reviewr"))
...
# Repo discovery; the launch-vs-live split is legacy/docs/herdr-api-notes.md). The mode guard
