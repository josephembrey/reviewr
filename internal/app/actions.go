...
// Compatibility names keep existing action-level tests and embeddings source
// compatible while routing now expresses the generalized fold/landmark intent.
const (
	ExpandReaderContext   = ExpandReaderFold
	CollapseReaderContext = CollapseReaderFold
	SelectNextHunk        = SelectNextLandmark
	SelectPreviousHunk    = SelectPreviousLandmark
)
