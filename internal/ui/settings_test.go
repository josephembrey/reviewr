...
func TestSettingsPopupRendersCenteredCheckboxAtMinimumSize(t *testing.T) {
	...
	for _, expected := range []string{"Settings · ,/esc close", "[x] include comments in hunk navigation ([/])"} {
	...
	if popupWidth != settingsPopupWidth || popupHeight != 3 {
		t.Fatalf("Settings popup = %dx%d, want %dx3", popupWidth, popupHeight, settingsPopupWidth)
	}
	if got, want := centeredPopupRect(geometry.Screen, popupWidth, popupHeight), (Rect{X: 3, Y: 4, Width: 54, Height: 3}); got != want {
