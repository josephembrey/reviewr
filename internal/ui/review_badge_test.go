...
func TestReviewBadgesUseIndependentAlignedRightSideField(t *testing.T) {
	...
	for _, state := range states {
		t.Run(state.Label(), func(t *testing.T) {
			row := NavigatorRow{Label: "main.go", Tree: true, Status: StatusModified, Reviewable: true, Review: state}
...
func TestNavigatorRowLayoutReservesProgressAndCompleteBadge(t *testing.T) {
	...
	if narrow := LayoutNavigatorRow(NavigatorRow{Reviewable: true, Review: review.Partial}, 4); narrow.Label.Width != 0 || narrow.Review.Width != 4 {
...
func TestChangedTreeRowRightAlignsLineStatsBeforeReviewBadge(t *testing.T) {
	changes := LineChanges{Additions: 12, Deletions: 3}
	row := NavigatorRow{Label: "main.go", Tree: true, Status: StatusModified, Changes: changes, Reviewable: true, Review: review.Reviewed}
...
func TestChangedTreeRowOmitsZeroLineStats(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			row := NavigatorRow{Label: "main.go", Tree: true, Status: StatusModified, Changes: test.changes, Reviewable: true, Review: review.Reviewed}
...
func TestEveryBadgeCellHitsReviewWithoutActivatingTheRow(t *testing.T) {
	...
	rows[3].Reviewable = true
	rows[3].Review = review.Unreviewed
	top := 2
