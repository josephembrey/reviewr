...
func TestHeaderControlsFollowActiveWorkspace(t *testing.T) {
	...
	if !strings.HasPrefix(files, "[files | g git | n notes] [all] [file] [uncommitted]") {
	...
	if !strings.HasPrefix(files, "[files | g git | n notes] [changed] [diff] [last-turn]") {
...
func TestWideHeaderControlsExposeNumberKeys(t *testing.T) {
	...
	if !strings.HasPrefix(plain, "[files | g git | n notes]  1 [all]  2 [file]  3 [uncommitted]") {
